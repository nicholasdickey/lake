...
export const fetchQueues = async ({ width, layout, qparams, session }: { width: number, layout: any, qparams: Qparams, session: Options }) => {
  ...
  for (let i = 0; i < qTypes.length; i++) {
    ...
    const key = ['queue', type, qparams.newsline, qparams.forum, qparams.tag, 0, 0, sessionid, session.userslug, ''] as unknown as [u: string, qType: string, newsline: string, forum: string, tag: string, page: number, lastid: string, sessionid: string, userslug: string, tail: string, test: string];
  ...
  if (session.band) {
    const key = ['queue', 'hot', qparams.newsline, qparams.forum, qparams.tag, 0, 0, sessionid, session.userslug, ''] as unknown as [u: string, qType: string, newsline: string, forum: string, tag: string, page: number, lastid: string, sessionid: string, userslug: string, tail: string, test: string];
