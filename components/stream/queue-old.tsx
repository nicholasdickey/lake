   ...
   /* const [isv, setIsv] = useState(visible)
    ...
    const setData = useCallback((data: any, pageIndex: number, hasData: boolean, tail: number) => {
        ...
        if (pageIndex >= segments.length - 1) {
            ...
            if(qType=='newsline')
             console.log("d1b: remder ---> adding segments for fetchData pageIndex:", guid, card, visible, pageIndex, qType, 'segments:', segments)
    ...
    const resetSegments = useCallback(() => {
       //if (!visible)
        ...
        if(qType=='newsline') 
         console.log(`d1b: resetSegments ${qType}`, { guid, visible, segments, oldSegments })
